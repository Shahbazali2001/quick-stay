  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-10 pb-30'>
        <div className='flex flex-col md:flex-row items-center justify-between w-full'>
                <Title align="left" title="Exclusive Offers" subTitle="Take advantage of our limited-time offers and special packages to enhance your stay and create unforgettable memories." font="font-playfair" />
                <button className='group flex items-center gap-3 px-4 py-2 text-sm font-medium hover:bg-gray-50 transition-all cursor-pointer'>
                    View All Offers
                ...
                {exclusiveOffers.map((item)=>(
                    <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
                        ...
                        <div>
                            ...
                            <p className='text-xs text-white/70 mt-3'>Expires in: {item.expiryDate}</p>
                        </div>
